...
/**
 * [API Mocking] 티맵 성격 기능 목킹 데모 (즐겨찾기 / 장소추천 / 경로검색 엣지케이스)
 * 채용공고의 "API Mocking" 요건 대응 + 모빌리티 도메인 시나리오 시연.
 *
 * 주의: 실제 티맵 API가 아니라, page.route로 만든 "가상(mock) 응답"에 대한 검증입니다.
 * 외부 네트워크에 의존하지 않아 CI에서 항상 결정적으로 통과합니다.
 *
 * 대응 TC: TC-MOCK-003 ~ TC-MOCK-006
 */

const APP_ORIGIN = 'https://tmap.demo';

test.describe('티맵 기능 API Mocking', () => {
  ...
  test.beforeEach(async ({ page }) => {
    await page.route(`${APP_ORIGIN}/`, async (route) => {
      await route.fulfill({
        ...
        body: '<!doctype html><html><body><h1>TMAP Demo</h1></body></html>',
      });
    });

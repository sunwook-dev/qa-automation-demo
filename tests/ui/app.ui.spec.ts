...
/**
 * [UI 자동화] 데모 앱 (로그인 → 경로검색)
 * POM(DemoAppPage) 기반. 화면·API를 목킹하여 외부 네트워크 없이 결정적으로 검증합니다.
 * 실제 서비스가 아니라, 동일한 검증 방식을 재현한 데모입니다.
 *
...
test.describe('데모 앱 - 로그인', () => {
  test('TC-UI-001 | 유효 계정으로 로그인하면 경로검색 화면으로 진입한다', async ({ page }) => {
    const app = new DemoAppPage(page);
    await app.open();
    await app.mockLogin(true);

    await app.login('demo_user', 'secret');
  ...
  test('TC-UI-002 | 잘못된 자격증명이면 오류 메시지를 표시한다', async ({ page }) => {
    ...
    await app.login('demo_user', 'wrong');
